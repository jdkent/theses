...
The positivity bias is lower in the real data relative to the null data for the mixed block/event contrasts,
providing further evidence suggesting the results represent a real context-dependent
correlation change.
...
(Table~\ref{table_bias}) agrees with previous work looking at
...
\begin{table}[H]
  ...
  Maintains expected false positive rate (in high activation/CNR ROI sets) & \xmark & \cmark & \xmark\\ \hline
  Maintains expected false positive rate (with full cortex coverage) & \xmark & \xmark & \xmark\\ \hline
