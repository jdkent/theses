...
Cognitive neuroscientists couple clever experiments with a measure of the brain
(such as functional Magnetic Resonance Imaging (fMRI)) to provide a nuanced
analysis of behavior and how that behavior correlates with measures of the brain.
The measures of the brain using fMRI typically look for which brain areas are more
active than other brain areas between two (or more) conditions in a task.
More recently, fMRI research has acknowledged the importance of understanding
...
While the methods to detect which brain areas are working together more during one condition
relative to another were developed around the turn of the century, they have primarily
been used for directed hypotheses within a small number of brain areas, or an
exporatory analyses observing how one brain area changes its relationship
with the rest of the brain.
These methods have not been extensively used to investigate large numbers of defined
brain areas which is the bedrock of analyzing brain networks.
To fill this gap, \textbf{I compared three different methods that measure brain networks during a task
...
Despite the limitations of the BOLD contrast, the connection between blood rushing to areas of the brain where 
neuronal activity occurs provided the backdrop for discovering the large scale brain networks
at rest and discovering which brain areas are active for specific tasks.
...
participant at rest.
It is the \textit{context-dependent functional connectivity} between brain areas observed during a task
...
For the context of this thesis, brain networks refer to which brain areas are strongly communicating
with each other, as measured by either a statistical or physical dependency between
regions like a Pearson's correlation or a white matter tract~\cite{Uddin2019}.
...
to test how similar the methods are and provide a foundation
...
An example of interleaved contexts would be performing a task where you typically
press a button corresponding to an arrow's directions, but sometimes a tone or visual cue
turns on telling you to withhold your response.
Withholding your response is a transient context that exists interleaved
with the other context of pressing the arrow as expected.
...
Broadly, this problem is known as \textit{collinearity} between $X_{apple}$ and $X_{orange}$. 
...
If Rissman chose a sliding window or residual approach to measure context-dependent functional connectivity,
...
detect differences in context-dependent functional connectivity when events from different conditions
are interleaved.
...
but several key differences as well.
While LSS maintained the expected number of false positives for the motivated selection of
ROIs for the activation and SchaeferTop20 atlases, LSS percentage of false positives is
higher than LSA when using the full Schaefer atlas.
PPI false positive percentage remained the highest in the full Schaefer atlas.
...
The reason for these discrepencies could be due to the differences in outcome measures used in their work and
this thesis.
...
(right hand side of equation~\ref{eq:neq_conv}) or
2) convolve the psychological variable with an HRF to represent BOLD signal
(left hand side of equation~\ref{eq:neq_conv}),
\begin{equation}
...
through centering the psychological variables or including the deconvolved/reconvolved physiological variable
in the model~\cite{Di2017}.
One of those solutions is needed because of the imperfections of the deconvolution process.
Deconvolution is not an invert-able process when the underlying neural signal is not known
so the reconvolved PPIs regressors are not perfectly represented at the BOLD level.
Not accounting for the imperfect deconvolution process results in the PPI regressors
containing information about the baseline functional connectivity between brain areas, biasing
PPI effects to detect regions are highly functionally connected at baseline.
The implementation used in this thesis work demeans the psychological variables.
...
The decrease of context-dependent functional connectivity seen in the contrasts
$Dual - Single$ and $Repeat - Single$ for the real data agrees with previous
work looking at context-dependent functional connectivity during various task states~\cite{Cole2014a,Spadone2015,Di2019a,He2013,Ito2019,Hearne2020}.
...
Additionally, there were often fewer significant results in the $Switch - Repeat$ contrast,
suggesting this particular sample happened to detect more increases than decreases in
...
\begin{table}[H]
  ...
  Maintains expected false positive rate (in small ROI sets) & \xmark & \cmark* & \xmark\\ \hline
  ...
  * depends on the choice of atlas
  \label{table:comparison}
\end{table}

When using an atlas that containing full coverage of the cortex, however, LSS appears to have
more false positives than LSA~\ref{fig:schaefer_binomial_ppi}.
