...
\begin{figure}[H]
  \centering
  \subfloat{\includegraphics[width=\textwidth]{data-real_atlas-schaefer_participants-filtered_permutation_summary}}

  \subfloat{\includegraphics[width=\textwidth]{data-null_atlas-schaefer_participants-filtered_permutation_summary}}
...
Second, providing the participant data was supported by Dr's. Hyun Kyu Lee (PI),
Michelle Voss (site-PI), and Chandramallika Basak (site-PI)funded by grant NIH AG047722.
Third, we thank Chris Oehler and Adriana-Rivera Dompencial for collecting the participant data used in the data analysis.

\section*{Author Contributions}
JK and MV conceptualized this work, CB and MV funded the study collecting the participant data,
...
2) deconvolution of an ROI time series,
3) centering of the psychological variable,
...
After all PPIs were created, the full regression model was represented as follows
...
\begin{itemize}
  \item list the 2x2 matrix of activation/cnr x real/null
  \item list correlation results
\end{itemize}
...
PPI correlated significantly more strongly with LSS relative to LSA
for all contrasts ($dual - single$, $repeat - single$, and $switch - repeat$),
atlases (activation atlas, SchaeferTop20 atlas, and the full Schaefer atlas),
and the real and null data except the $switch - repeat$ contrast in the SchaeferTop20 Atlas,
($p < 0.05$).
The average correlation difference between LSS+PPI versus LSA+PPI is 0.26,
suggesting a medium effect size.
The similarity between LSS and PPI is stronger for the block contrasts relative
to the event-related contrast, regardless of data (real/null) or atlas.


\section{Discussion}
We provided the first in-depth comparison between LSA, LSS, and PPI.
We found while PPI generally found more results in the real data,
...
you can be more confident with the results.

\subsection{Limitations}

There are a number of limitations 
\begin{itemize}
  \item LSS is more correlated with PPI than LSA
  \item PPI is more sensitive, but less specific
  \item the spatial profile of results appears to be different between real and null data
  \item agrees with Xin Di's results
  \item recommend to use PPI for event related contrasts
  \item recommend to use LSS/PPI for block contrasts
  \item improve implementation of PPI
  \item choice of atlas matters
  \item choosing high CNR regions
\end{itemize}
%=============================================================================
